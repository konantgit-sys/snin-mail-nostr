...
def test_finish_without_lease_rejected(queue_db):
    """H14: завершение без lease отклоняется — чужой/бесхозный воркер не может
    закрыть задачу, которую взял другой (защита от двойного завершения)."""
    qid = q.enqueue(_ev("A" * 64))
    row = q.claim(groups={"A" * 64}, worker="w1")
    assert q.finish(row["id"], True) is False          # без lease — не владелец
    with sqlite3.connect(queue_db) as c:
        status = c.execute("SELECT status FROM mail_queue WHERE id=?", (qid,)).fetchone()[0]
    assert status == "processing"                       # задача не закрыта
    assert q.finish(row["id"], True, lease=row["lease"]) is True   # владелец закрывает
    with sqlite3.connect(queue_db) as c:
        status = c.execute("SELECT status FROM mail_queue WHERE id=?", (qid,)).fetchone()[0]
    assert status == "done"
