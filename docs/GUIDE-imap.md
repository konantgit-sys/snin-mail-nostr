...
## Ограничение окружения: наш контейнер (2026-09-10)

Проверено живым подключением, не по документации: с этого сервера порт
**993 закрыт** — `imap.mail.ru:993` и `imap.yandex.ru:993` обрывают TLS за
~50 мс (`SSLZeroReturnError: TLS/SSL connection has been closed (EOF)`).
Исходящий трафик идёт через управляемый прокси, который пропускает
HTTP/HTTPS, но не почтовые порты.

Что из этого следует:

- мост в этом окружении **не может забрать обычную почту** ни из mail.ru,
  ни из Яндекса — это ограничение сети, а не кода;
- единственный конфиг в `imap_configs` выключен (`enabled=0`), в
  `last_error` — та же ошибка TLS;
- решение отложено до переезда на **другой сервер** с открытым 993
  (см. `DEPLOY.md`: «port 993/995 must be allowed on the server»);
- обходной путь без IMAP: пересылка писем сервисом, который отдаёт их по
  HTTPS на наш webhook (`*.v2.site`) — тогда мост получает готовый текст
  и публикует `kind:1301`, не открывая почтовых портов.
