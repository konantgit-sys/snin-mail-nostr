...
## Контракт очереди (2026-09-10)

Две гарантии, добавленные после живого потока чужих событий (kind 25910 —
JSON-RPC чужого протокола, приходил на наши ключи из NIP-05-списка):

1. **Постоянный отказ не повторяется.** `finish(..., permanent=True)`
   уходит в `failed` сразу, без трёх попыток: повтор не превратит чужой
   kind в письмо. Мост сообщает постоянность через `last_error_permanent`
   (чужой kind, неразбираемый контент, нет From/Subject, дубликат,
   невалидная подпись, нет нашего p-тега). **Полный ящик — временная
   причина**: письмо ждёт места, попытки не сжигаются.
2. **Письмо на незарегистрированный ящик не занимает очередь.**
   `enqueue()` отбрасывает событие, если получателя нет в `accounts`
   (ящик снят владельцем или никогда не создавался). Событие на снятый
   ключ не попадает в `mail_queue` вообще — ни `pending`, ни `failed`.

Причина правки: ящик `label='x'` был зарегистрирован на публично
известный тестовый ключ (приватный ключ = 1) и собирал 161 отклонённую
задачу от чужого протокола. Ящик снят, задачи выгружены в бэкап-дамп и
удалены, поток не доходит до очереди.
